      ...
      <div class="card-header bg-primary">
        <div class="row align-items-center">
          ...
          <div class="col-6 text-right">
            <a href="{{ route('admin.apbdes.index') }}" type="button" class="btn btn-warning float-end">Kembali</a>
          </div>
        </div>
      </div>
    </div>
...
<form method="POST" action="{{ route('admin.apbdes.store') }}" enctype="multipart/form-data">
@csrf
<div class="row">
  <div class="col-lg-8">
    <div class="card shadow-lg">
      <div class="card-body">
        ...
        <div class="mb-3">
          ...
          <textarea class="form-control" id="editor" name="keterangan" rows="10" required>{{ old('keterangan') }}</textarea>
        ...
        <div class="mb-3">
          ...
          <small class="text-muted d-block mt-1">Maks 20MB. Format: PDF, XLS/XLSX/CSV, DOC/DOCX, PPT/PPTX, ZIP/RAR.</small>
...
{{-- Slug otomatis (endpoint APBDes) --}}
<script>
  const judul = document.querySelector('#judul');
  const slug  = document.querySelector('#slug');
  if (judul) {
    judul.addEventListener('change', function(){
      fetch('{{ route('admin.apbdes.slug') }}?judul=' + encodeURIComponent(judul.value))
        .then(r => r.json())
        .then(d => slug.value = d.slug)
        .catch(() => {});
    });
  }
</script>
